...
namespace VelastoProductionSystem.Controllers
{
    public class SpsMasterController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var data = await _context.SpsMasters.ToListAsync();
        ...
        public async Task<IActionResult> Details(int? id)
        {
            ...
            var item = await _context.SpsMasters
        ...
        public async Task<IActionResult> Edit(int? id)
        {
            ...
            var item = await _context.SpsMasters.FindAsync(id);
            if (item == null) return NotFound();
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, SpsMaster item)
        {
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            if (item != null)
            {
                _context.SpsMasters.Remove(item);
                await _context.SaveChangesAsync();
        ...
        public async Task<IActionResult> DeleteMultiple([FromBody] int[] ids)
        {
            ...
            try
            {
                var itemsToDelete = await _context.SpsMasters
                    .Where(m => ids.Contains(m.Id))
                    .ToListAsync();

                if (itemsToDelete.Any())
                {
                    _context.SpsMasters.RemoveRange(itemsToDelete);
                    await _context.SaveChangesAsync();
                }
        ...
        {
            return _context.SpsMasters.Any(e => e.Id == id);
        }

        // GET: SpsMasters/DownloadEmptyTemplate - Template KOSONG format baru MIN|ASLI|MAX
        [HttpGet]
        public IActionResult DownloadEmptyTemplate()
        {
            using var package = new OfficeOpenXml.ExcelPackage();
            var ws = package.Workbook.Worksheets.Add("SPS Master Template");

            // --- Title ---
            ws.Cells[1, 1].Value = "SPS MASTER TEMPLATE - Format MIN|ASLI|MAX (216 Kolom KOSONG)";
            ...
            ws.Cells[2, 1].Value = $"Template Kosong | Download Date: {DateTime.Now:dd MMM yyyy HH:mm} | Isi data mulai dari Row 4";
            ...
            // Build headers sama seperti ExportExcel
            ...
            // Add tolerance columns (171 kolom)
            ...
            {
                headersList.Add($"{field}_MIN"); headersList.Add($"{field}_ASLI"); headersList.Add($"{field}_MAX");
            }
            
            for (int i = 1; i <= 3; i++)
            {
                headersList.Add($"MeshDim{i}_MIN"); headersList.Add($"MeshDim{i}_ASLI"); headersList.Add($"MeshDim{i}_MAX");
            }
            
            for (int i = 1; i <= 3; i++)
            {
                headersList.Add($"HeadTemp{i}_MIN"); headersList.Add($"HeadTemp{i}_ASLI"); headersList.Add($"HeadTemp{i}_MAX");
            }
            ...
            for (int ext = 1; ext <= 3; ext++)
            {
                ...
                {
                    headersList.Add($"Cylinder{ext}_{mat}_MIN"); headersList.Add($"Cylinder{ext}_{mat}_ASLI"); headersList.Add($"Cylinder{ext}_{mat}_MAX");
                }
            }
            
            for (int i = 1; i <= 3; i++)
            {
                headersList.Add($"ScrewTemp{i}_MIN"); headersList.Add($"ScrewTemp{i}_ASLI"); headersList.Add($"ScrewTemp{i}_MAX");
            }
            
            headersList.Add("ChillerWaterTemp_MIN"); headersList.Add("ChillerWaterTemp_ASLI"); headersList.Add("ChillerWaterTemp_MAX");
            
            var outputFields = new[] { "ScrewSpeed1", "ScrewSpeed2", "ScrewSpeed3", "Pressure1", "Pressure2", "Pressure3",
                "Feed1", "Feed2", "Feed3", "FeedRollRatio1", "FeedRollRatio2", "FeedRollRatio3",
                "HoseSpeed", "TakeUpConveyorSpeed", "CoolConveyorSpeed", "ConveyorRatio",
                "SpiralPitchSetting", "SpiralSpeed", "PresetValue", "ControlValue",
                "AmMeter1", "AmMeter2", "AmMeter3", "CaterpillarGap", "UnsmoothSurface" };
            ...
            // Write headers ke Row 3
            for (int i = 0; i < headersList.Count; i++)
            {
                ws.Cells[3, i + 1].Value = headersList[i];
                ws.Cells[3, i + 1].Style.Font.Bold = true;
                ws.Cells[3, i + 1].Style.Fill.PatternType = OfficeOpenXml.Style.ExcelFillStyle.Solid;
                ws.Cells[3, i + 1].Style.Fill.BackgroundColor.SetColor(System.Drawing.Color.FromArgb(70, 130, 180));
                ws.Cells[3, i + 1].Style.Font.Color.SetColor(System.Drawing.Color.White);
                ws.Cells[3, i + 1].Style.HorizontalAlignment = OfficeOpenXml.Style.ExcelHorizontalAlignment.Center;
                ws.Cells[3, i + 1].Style.Border.BorderAround(OfficeOpenXml.Style.ExcelBorderStyle.Thin);
            }
            ...
            return File(content, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", 
                $"SPS_Master_Template_Empty_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx");
        }
        ...
        {
            var data = await _context.SpsMasters
            ...
            ws.Cells[2, 1].Value = $"Export Date: {DateTime.Now:dd MMM yyyy HH:mm} | Total: {data.Count} records | Format: MIN|ASLI|MAX for all tolerance fields";
            ws.Cells[2, 1, 2, 50].Merge = true;
            ws.Cells[2, 1].Style.HorizontalAlignment = OfficeOpenXml.Style.ExcelHorizontalAlignment.Center;
            ws.Cells[2, 1].Style.Font.Italic = true;
            ws.Cells[2, 1].Style.Font.Color.SetColor(System.Drawing.Color.Gray);
            ...
            for (int i = 1; i <= 3; i++)
            {
                ...
                headersList.Add($"MeshDim{i}_ASLI");
            ...
            for (int i = 1; i <= 3; i++) // HeadTemp 1-3
            {
                ...
                headersList.Add($"HeadTemp{i}_ASLI");
            ...
            for (int ext = 1; ext <= 3; ext++)
            {
                for (int mat = 1; mat <= 3; mat++)
                {
                    ...
                    headersList.Add($"Cylinder{ext}_{mat}_ASLI");
            ...
            for (int i = 1; i <= 3; i++) // ScrewTemp 1-3
            {
                ...
                headersList.Add($"ScrewTemp{i}_ASLI");
                headersList.Add($"ScrewTemp{i}_MAX");
            }
            headersList.Add("ChillerWaterTemp_MIN");
            headersList.Add("ChillerWaterTemp_ASLI");
            ...
            try
            {
                using (var stream = new MemoryStream())
                {
                    ...
                    using (var reader = ExcelReaderFactory.CreateReader(stream))
                    {
                        ...
                        int startRow = 6;
                        var existingData = await _context.SpsMasters.ToListAsync();
                        ...
                        for (int i = startRow; i < table.Rows.Count; i++)
                        {
                            ...
                            foreach (var itemCode in itemsList)
                            {
                                ...
                                if (isNew) { _context.SpsMasters.Add(target); newCount++; }
                                else { _context.SpsMasters.Update(target); updateCount++; }
