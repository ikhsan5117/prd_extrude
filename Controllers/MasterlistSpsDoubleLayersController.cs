                ...
                using (var stream = new MemoryStream())
                {
                    ...
                    using (var reader = ExcelReaderFactory.CreateReader(stream))
                    {
                        ...
                        for (int i = 0; i < dataTable.Rows.Count; i++)
                        {
                            ...
                            if (string.IsNullOrEmpty(idExcel) || idExcel.Equals("ID", StringComparison.OrdinalIgnoreCase) || 
                            ...
                            foreach (var item in items)
                            {
                                var newItem = new MasterlistSpsDoubleLayer
                                {
                                    ...
                                    Yarn = GetV(row, 12),
                                    OuterCover = GetV(row, 13),
                                    UseLimitsInner = GetV(row, 14),
                                    UseLimitsOuter = GetV(row, 15),
                                    Nipple = GetV(row, 16),
                                    TubeDie = GetV(row, 17),
                                    CoverDie = GetV(row, 18),
                                    MeshScreen1 = GetV(row, 19),
                                    MeshScreen2 = GetV(row, 20),
                                    HeadTemp1 = GetV(row, 21),
                                    HeadTemp2 = GetV(row, 22),
                                    Cylinder1_1 = GetV(row, 23),
                                    Cylinder1_2 = GetV(row, 24),
                                    Cylinder2_1 = GetV(row, 25),
                                    Cylinder2_2 = GetV(row, 26),
                                    Feed1 = GetV(row, 27),
                                    Feed2 = GetV(row, 28),
                                    ScrewTemp1 = GetV(row, 29),
                                    ScrewTemp2 = GetV(row, 30),
                                    ScrewSpeed1 = GetV(row, 31),
                                    ScrewSpeed2 = GetV(row, 32),
                                    Pressure1 = GetV(row, 33),
                                    Pressure2 = GetV(row, 34),
                                    AmMeter = GetV(row, 35),
                                    OdSensor = GetV(row, 36),
                                    MarkingSort = GetV(row, 37),
                                    TextMarkingMaterial = GetV(row, 38),
                                    MarkingColour = GetV(row, 39),
                                    ChillerWaterTemp = GetV(row, 40),
                                    CuttingSpeed = GetV(row, 41),
                                    TakeUpConveyorSpeed = GetV(row, 42),
                                    ToleranceInner = GetV(row, 43),
                                    ToleranceOuter = GetV(row, 44),
                                    TebalInner = GetV(row, 45),
                                    TebalOuter = GetV(row, 46),
                                    TebalTotal = GetV(row, 47),
                                    SelisihTebal = GetV(row, 48),
