...
class Program
{
    static void Main()
    {
        ...
        var excelDocs = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
        ...
        foreach (var file in files)
        {
            ...
            // Find machine column index for the current file
            int machineCol = 3; // Default is column 3 for most files
            ...
            for (int r = 1; r <= rows; r++)
            {
                for (int c = 1; c <= cols; c++)
                {
                    ...
                    // Collect doc numbers (Exclude EPR since user only uses SPS)
                    if (v.StartsWith("SOP/PROD") && !v.Contains("/EPR/"))
                    {
                        var machine = ws.Cells[r, machineCol].Text?.Trim() ?? "-";
                        if (!excelDocs.ContainsKey(v))
                        {
                            excelDocs[v] = machine;
                        }
                    }
        ...
        var missingDocs = excelDocs.Keys.Except(dbDocs).OrderBy(x => x).ToList();
        ...
        if (missingDocs.Any())
        {
            Console.WriteLine("\n[No. Dokumen yang BELUM ADA di Database beserta Mesinnya]");
            foreach (var d in missingDocs) 
            {
                Console.WriteLine($"- Dokumen: {d} | Mesin: {excelDocs[d]}");
            }
        }
