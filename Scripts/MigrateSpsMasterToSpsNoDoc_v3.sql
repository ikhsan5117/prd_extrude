-- MigrateSpsMasterToSpsNoDoc_v3.sql
BEGIN TRANSACTION;

-- 1. Insert ke SpsNoDocs (Group berdasarkan DocumentNumber, Machine, dan Formulasi supaya '-' tidak tercampur)
...
SELECT 
    MAX(ExcelId), MAX(No), ISNULL(Machine, ''), ISNULL(DocumentNumber, ''), MAX(RevisionNumber), MAX(Customer), MAX(RevisionDate), ISNULL(Formulasi, ''), MAX(HoseType), MAX(Dimensi), MAX(Material),
...
FROM SpsMasters
GROUP BY ISNULL(DocumentNumber, ''), ISNULL(Machine, ''), ISNULL(Formulasi, '');

-- 2. Insert ke SpsItemLists
INSERT INTO SpsItemLists (ItemList, SpsNoDocId)
SELECT m.ItemList, n.Id
FROM SpsMasters m
JOIN SpsNoDocs n ON ISNULL(n.DocumentNumber, '') = ISNULL(m.DocumentNumber, '')
                 AND ISNULL(n.Machine, '') = ISNULL(m.Machine, '')
                 AND ISNULL(n.Formulasi, '') = ISNULL(m.Formulasi, '')
